...
# docker exec -it `docker ps -q` bash /data/llamafactory/run.sh 0 2>&1|tee pt_lorarank8_zero2_noderank0_gpu64.log
# docker exec -it `docker ps -q` bash /data/llamafactory/run.sh 1 2>&1|tee pt_lorarank8_zero2_noderank1_gpu72.log
